    ...
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-end" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-200 px-6 sm:px-8 py-5 flex items-center justify-between z-10">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">编辑课堂</h2>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5">修改课堂信息、结果权限与完成提示</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
              课堂说明 <span className="text-gray-400 text-xs normal-case">（选填）</span>
            </label>
            <textarea
              id="description"
              value={description}
          ...
          <div className="border-t border-gray-200 pt-6">
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">结果访问权限</h3>
            <div className="space-y-3">
              <label className="flex items-start gap-3 cursor-pointer p-4 border-2 border-gray-200 rounded-xl hover:border-blue-300 transition-all group">
                ...
                <div className="flex-1">
                  <div className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">公开结果</div>
                  <div className="text-sm text-gray-600">获得结果链接的用户均可查看</div>
                </div>
              </label>
              <label className="flex items-start gap-3 cursor-pointer p-4 border-2 border-gray-200 rounded-xl hover:border-blue-300 transition-all group">
                <input
                ...
                />
                <div className="flex-1">
                  <div className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">私密结果</div>
                  <div className="text-sm text-gray-600">查看结果时需要输入 4 位访问码</div>
                </div>
              </label>
              {!resultsPublic && (
          ...
          <div className="border-t border-gray-200 pt-6">
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">界面样式</h3>
          ...
          {/* Completion Message */}
          <div className="border-t border-gray-200 pt-6">
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">完成提示</h3>
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
                  主标题 <span className="text-gray-400 text-xs normal-case">（选填）</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-base"
                  placeholder="例如：本轮课堂检测已完成"
                />
              </div>
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
                  副标题 <span className="text-gray-400 text-xs normal-case">（选填）</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-base"
                  placeholder="例如：你的回答已经保存"
                />
              </div>
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
                  补充说明 <span className="text-gray-400 text-xs normal-case">（选填）</span>
                </label>
                <textarea
                  value={completionDescription}
                  onChange={(e) => setCompletionDescription(e.target.value)}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-base resize-none"
                  placeholder="可填写后续安排、复测提示或教师说明"
                  rows={3}
                />
              </div>
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
                  完成页图片 <span className="text-gray-400 text-xs normal-case">（选填）</span>
                </label>
                <input
                  ref={fileInputRef}
                  type="file"
                ...
                {session.completionImageId && !completionImageFile && (
                  <p className="text-xs text-gray-500 mt-1">如不选择新图片，将保留当前图片</p>
                )}
              </div>
            </div>
          </div>
