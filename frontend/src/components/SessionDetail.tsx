...
export function SessionDetail({ session, onBack }: SessionDetailProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(`成功导入 ${importedElements.length} 道题目`);
    } catch (error: any) {
      toast.error(`题目导入失败：${error.message || '文件格式无效'}`);
    } finally {
      // Reset file input
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-4 sm:px-0">
      ...
      <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-2xl p-4 sm:p-6 shadow-sm">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-blue-900">课堂码：{session.sessionCode}</h3>
          </div>
          <svg 
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-4 overflow-hidden transition-all duration-300 ${
          ...
          <div className="bg-white/80 rounded-xl p-4 space-y-3 border border-blue-200">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-base font-bold text-gray-900">学生作答</h4>
            </div>
            <div>
          ...
          <div className="bg-white/80 rounded-xl p-4 space-y-3 border border-blue-200">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-base font-bold text-gray-900">课堂结果</h4>
      ...
      <LearningMaterialPanel sessionId={session._id} />

      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Micro Checks（{elements.length}）</h2>
        <button
      ...
      {elements.length === 0 ? (
        <div className="text-center py-12 sm:py-16 bg-white rounded-2xl border-2 border-dashed border-gray-300">
          <div className="max-w-md mx-auto px-4">
            ...
            <p className="text-gray-600 text-lg mb-6">还没有 Micro Check</p>
            <button
              onClick={() => setShowCreateModal(true)}
