...
export function EditElementModal({ element, onClose }: EditElementModalProps) {
  ...
  const handleCsvUpload = async (file: File) => {
    try {
      ...
      toast.success(`已从 CSV 导入 ${newChoices.length} 个选项`);
    } catch (error) {
      toast.error("CSV 文件解析失败，请检查格式");
    }
  };
  ...
  const getElementTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "text_input": return "文本回答";
      case "number_input": return "数值滑块";
    ...
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-end" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-200 px-8 py-5 flex items-center justify-between z-10">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">编辑题目</h2>
            <p className="text-sm text-gray-500 mt-0.5">修改题目内容与答案设置</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500 mt-1">题目创建后不能修改题型</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-base"
              placeholder="补充题目情境或知识点"
            />
          </div>

          <div className="space-y-2">
            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
              作答说明 <span className="text-gray-400 text-xs normal-case">（选填）</span>
            </label>
            <textarea
              value={description}
          ...
          <div className="space-y-2">
            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wider">
              题目图片 <span className="text-gray-400 text-xs normal-case">（选填）</span>
            </label>
            {element.imageUrl && !imageFile && (
              <div className="mb-3 p-3 bg-gray-50 rounded-xl border border-gray-200">
                <img src={element.imageUrl} alt="当前题目图片" className="max-w-xs max-h-32 object-cover rounded-lg" />
                <p className="text-xs text-gray-500 mt-2">当前图片 · 上传新图片后将替换</p>
              </div>
            )}
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">（仅读取第一列）</span>
                </div>
              </div>
                  ...
                  <div key={choice.id} className="flex gap-3 items-start p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <label className="flex items-center gap-2.5 text-sm cursor-pointer group">
                        ...
                        />
                        <span className="text-gray-600 group-hover:text-gray-900 transition-colors">设为正确答案</span>
                      </label>
                    </div>
                  </div>
                ))}
